...
def scrape_articles_from_category(url, category):
    ...
    try:
        ...
        save_cookies(driver, "session_cookies_" + category + ".pkl")
    ...
    while True:
        try:
            ...
            if article is None:
                ...
                break
            try:
                print("Début du TTT de l'article : ", article.get("url"))
                scrap_article(driver, article.get('url'), cat)
                print("Fin du TTT de l'article : ", article.get("url"))
            except Exception:
                print("Erreur du TTT de l'article : ", article.get("url"))
                continue
            article_queue.task_done()
        except:
            break
